...
static const wilc_Uint8  g_bmap[8] = {1, 2, 4, 8, 16, 32, 64, 128}; /* Bit map */
...
wilc_Uint8   g_snap_header[SNAP_HDR_ID_LEN] = {0xAA, 0xAA, 0x03, 0x00, 0x00, 0x00};

/* Power management related globals */
wilc_Uint8       g_num_sta_ps              = 0; /* Num of STA in PS */
wilc_Uint32      g_num_mc_bc_pkt           = 0; /* Num of BC/MC pkts in PSQ     */
wilc_Uint32		 g_num_ps_pkt     		   = 0; /* Num of UC/BC/MC pkts in PSQ  */
wilc_Uint16      g_num_mc_bc_qd_pkt    	   = 0; /* Num of BC/MC pkts in Hw  	*/
list_buff_t  g_mc_q;
//wilc_Uint8       g_vbmap[VBMAP_SIZE]       = {0,};
//wilc_Uint8       g_tim_element_index       = 0;
//wilc_Uint16      g_tim_element_trailer_len = 0;
q_head_t     g_ps_pending_q            = {0,};
wilc_Uint16      g_aging_thresh_in_sec     = AGING_THR_IN_SEC;
wilc_ieee80211_sta *g_max_ps_ae = NULL;      /* Association entry of STA, with max num of PS pkts */


void* find_entry(wilc_Uint8* key);
void add_entry(void* entry, wilc_Uint8* key);
void delete_entry(wilc_Uint8* key);
void frmw_to_linux(uint8_t *buff, uint32_t size);
void ap_enabled_rx_data( wilc_Uint8 *msg);
REQUEUE_STATUS_T requeue_ps_packet(wilc_ieee80211_sta* ae, list_buff_t *qh,
                                   wilc_Bool ps_q_legacy, wilc_Bool eosp);
wilc_Bool buffer_tx_packet(wilc_Uint8 *entry, wilc_Uint8 *da, wilc_Uint8 priority,
                         struct tx_complete_data *tx_dscr);
void handle_ps_tx_comp_ap(struct tx_complete_data *tx_dscr );
void check_and_reset_tim_bit(wilc_Uint16 asoc_id);
wilc_Bool handle_ps_poll(wlan_rx_t    *wlan_rx );


/* This function returns whether the Multicast bit is set in the given beacon*/
/* frame                                                                     */
inline wilc_Bool get_mc_bit_bcn(void)
{
    wilc_Bool ret_value;
    ret_value = (wilc_Bool)(strBeaconInfo.u8beacon_frame[strBeaconInfo.u8tim_element_index + BMAP_CTRL_OFFSET] &
            0x01);
    return (ret_value);
}
...
*  @version		1.0
*/	
static void wilc_beacon_tx_complete(void* priv, int status)
{
	 // Do nothing 
...
*  @version		1.0
*/	
static int wilc_beacon_xmit(const u8 *buf, size_t len)
{
	...
	//wilc_PRINTF("--IN beacon_tx: Sending beacon Pkt to tx queue--\n");
	wilc_wlan_txq_add_mgmt_pkt(strBeacon_tx,strBeacon_tx->buff,strBeacon_tx->size,wilc_beacon_tx_complete);
									 ...
									 wilc_Uint32 u32TailLen, wilc_Uint8* pu8Tail)
{
	...
	wilc_Bool bIsBeaconSet = wilc_FALSE;
	wilc_Uint8 *pu8BeaconFrame;
	wilc_Uint16  u16size=0;
	wilc_wlan_dev_t* strWlan = Get_wlan_context(&u16size); 

	if(u16size != sizeof(wilc_wlan_dev_t))
		PRINT_ER("size of wilc_wlan_dev_t in wilc_wlan != it's size in wilc_HOST_AP\n");


	//wilc_PRINTF("--IN host_add_beacon--\n");
	...
	if(strBeaconInfo.u8beacon_frame == NULL)
	{
		...
		strBeaconInfo.u8beacon_frame  = (wilc_Uint8 *)kmalloc(strBeaconInfo.u16beacon_len,GFP_ATOMIC);
		if(strBeaconInfo.u8beacon_frame  == NULL)
		{
	...
	wilc_memcpy(pu8BeaconFrame, pu8Head, u32HeadLen);
	...
	*(pu8BeaconFrame++) = 0;
	...
	wilc_memcpy(pu8BeaconFrame, pu8Tail, u32TailLen);
	pu8BeaconFrame+=u32TailLen;

	wilc_beacon_xmit(strBeaconInfo.u8beacon_frame,strBeaconInfo.u16beacon_len);
...
void process_tbtt_isr(void)
{
       ...
       wilc_Uint16 i           = 0;
	wilc_Uint8 u8OldTimLen = 0;
	wilc_Uint32 dtim_count  = 0;
	wilc_Uint16  u16size=0;
	wilc_wlan_dev_t* strWlan = Get_wlan_context(&u16size); 
	...
	strWlan->hif_func.hif_read_reg(rMAC_DTIM_COUNT_ADDR, &dtim_count);
       ...
       //wilc_PRINTF("dtim_count = %d",dtim_count);
        ...
        if(dtim_count == 0)
        {
            ...
            /* set                                                           */
            if(wilc_TRUE == get_mc_bit_bcn())
                while(requeue_ps_packet(NULL, &g_mc_q, wilc_TRUE, wilc_FALSE)
                      == PKT_REQUEUED);
	...
	if(strBeaconInfo.u8beacon_frame != NULL)
	{
		...
		{
			wilc_Uint32 u32ShiftOffset = u8OldTimLen - strBeaconInfo.u8vbmap[LENGTH_OFFSET];
			wilc_Uint32 u32TrailerIndex = strBeaconInfo.u8tim_element_index+u8OldTimLen+2;
		...
		{
			wilc_Uint32 u32ShiftOffset = strBeaconInfo.u8vbmap[LENGTH_OFFSET] - u8OldTimLen;
			wilc_Uint32 u32TrailerIndex = strBeaconInfo.u8tim_element_index+u8OldTimLen+2;
...
inline void init_list_buffer(list_buff_t *lbuff, wilc_Uint32 lnk_byte_ofst)
{
    lbuff->head          = NULL;
    lbuff->tail          = NULL;
    lbuff->count         = 0;
...
inline void add_list_element(list_buff_t *lbuff, void *elem)
{
    ...
    if(lbuff->tail == NULL)
        ...
        NEXT_ELEMENT_ADDR(lbuff->tail, lbuff->lnk_byte_ofst) = (wilc_Uint32)elem;
                                    ...
                                    void *curr_el)
{
    ...
    {
        NEXT_ELEMENT_ADDR(prev_el, lbuff->lnk_byte_ofst) = (wilc_Uint32)next_el;
    }
...
/*!
 *  @fn		void wilc_AP_AddSta(u8 *mac, struct station_parameters *params)
...
{
	wilc_Uint8 au8Sa[6];
	wilc_Uint8 * pu8CurrData;
	wilc_ieee80211_sta* pstrAsocEntry = NULL;
	wilc_Uint32 i;
	wilc_Bool bIsPrevAsoc = wilc_FALSE;
	wilc_Uint8 ht_info_mod = 0;
	wilc_Uint16 u16CapInfo = 0;
	
	
	wilc_memcpy(au8Sa, mac, ETH_ALEN);

	PRINT_D(HOSTAPD_DBG,"Add station BSSID: %x, %x, %x,%x,%x,%x,\n",
			au8Sa[0], au8Sa[1], au8Sa[2], au8Sa[3], au8Sa[4], au8Sa[5]);

	pstrAsocEntry = (wilc_ieee80211_sta*)find_entry(au8Sa);


	pstrAsocEntry = (wilc_ieee80211_sta *)kmalloc(sizeof(wilc_ieee80211_sta),GFP_ATOMIC);	
	...
	/* Reset the AE Handle */
	wilc_memset(pstrAsocEntry, 0, sizeof(wilc_ieee80211_sta));
	...
	*/
	...
	pstrAsocEntry->ps_poll_rsp_qed  = wilc_FALSE;
	...
	{
		wilc_Uint8 min = (ae->op_rates.rates[i] & 0x7F);
		wilc_Uint8 mid = i;
		wilc_Uint8 tmp = 0;
	...
	{
		wilc_Bool found = wilc_FALSE;
		wilc_Uint8 mac_br = get_mac_basic_rate(i);
	...
	pstrAsocEntry->ht_cap.ht_supported = (params->ht_capa != wilc_NULL);
	PRINT_D(HOSTAPD_DBG,"HT supported: %d\n", pstrAsocEntry->ht_cap.ht_supported);
	if(pstrAsocEntry->ht_cap.ht_supported)
	{
	/*
	pu8CurrData += wilc_AP_ParseHTCap(pu8CurrData, &pstrAsocEntry->ht_hdl,
	...
	//{
		...
		/*if(get_11n_op_mode() == HT_MIXED_MODE)
		{
			...
			/*if((is_autoprot_enabled() == wilc_TRUE) &&
	...
	//pstrAsocEntry->tx_rate_mbps = get_user_rate((wilc_Uint8)
	...
	else
	{
		...
		if((pstrAsocEntry->state == ASOC) && (pstrAsocEntry->short_slot == 0))
		{
			...
			{
				if(g_short_slot_allowed == wilc_TRUE)
				{
					enable_short_slot();
	...
	/* UNSUP_RATE and not allow the station to associate with AP.        */
		...
		if((u16CapInfo & SHORTPREAMBLE) != SHORTPREAMBLE)
		{
			...
			if((pstrAsocEntry->state != ASOC) || (pstrAsocEntry->short_preamble == 1))
			{
				...
				g_short_preamble_enabled = wilc_FALSE;
		...
		else
		{
			...
			if((pstrAsocEntry->state == ASOC) && (pstrAsocEntry->short_preamble == 0))
			{
				...
				{
					g_short_preamble_enabled = wilc_TRUE;
...
/*!
 *  @fn		void wilc_AP_EditSta(u8 *mac, struct station_parameters *params)
...
inline void wilc_AP_EditSta(u8 *mac, struct station_parameters *params)
{
	wilc_Uint8 au8Sa[6];
	wilc_ieee80211_sta* pstrAsocEntry = NULL;
	wilc_Uint8 u8NumRatesOld = 0;
	wilc_Uint16 u16Mask, u16Set;
...
/*!
 *  @fn			inline void wilc_AP_RemoveSta(wilc_Uint8 * pu8StaInfo)
...
inline wilc_Bool is_snap_header_present(wlan_rx_t *strWlan_rx)
{
    wilc_Uint8 *data = NULL;
    ...
    if(wilc_memcmp(data, g_snap_header, SNAP_HDR_ID_LEN) != 0)
            return wilc_FALSE;
...
inline void set_frame_control(wilc_Uint8* header, wilc_Uint16 fc)
{
   header[0] = (wilc_Uint8)(fc & 0x00FF);
   header[1] = (wilc_Uint8)(fc >> 8);
}

inline wilc_Uint8 set_mac_hdr(wilc_Uint8 *mac_hdr)
{
...
inline wilc_Uint8 set_mac_hdr_prot(wilc_Uint8 *mac_hdr, wilc_Uint8 priority,
                               wilc_Uint8 service_class, wilc_Bool qos, wilc_Bool is_ht,
                               wilc_Uint8 is_amsdu)
{
	// TODO: implement 11n
#if 0
...
inline void set_from_ds(wilc_Uint8* header, wilc_Uint8 from_ds)
{
    header[1] &= 0xFD;
    header[1] |= (from_ds << 1);
}
...
/* ignored.                                                                  */
inline  void set_address1(wilc_Uint8* msa, wilc_Uint8* addr)
{
    wilc_memcpy(msa + 4, addr, ETH_ALEN);
}

/* This function sets the 'address2' field in the MAC header of the input    */
/* frame to the input MAC Address 'addr'. The 16 LSB bits of 'addr' are      */
/* ignored.                                                                  */
inline  void set_address2(wilc_Uint8* msa, wilc_Uint8* addr)
{
    wilc_memcpy(msa + 10, addr, ETH_ALEN);
}
...
/* frame to the input MAC Address 'addr'. The 16 LSB bits of 'addr' are      */
/* ignored.                                                                  */
inline  void set_address3(wilc_Uint8* msa, wilc_Uint8* addr)
{
    wilc_memcpy(msa + 16, addr, ETH_ALEN);
}


/* This function extracts the MAC Address in 'address1' field of the MAC     */
/* header and updates the MAC Address in the allocated 'addr' variable.      */
inline void get_address1(wilc_Uint8* msa, wilc_Uint8* addr)
{
    wilc_memcpy(addr, msa + 4, ETH_ALEN);
}

/* This function returns the pointer to the MAC Address in 'Address-1" field */
/* of the MAC header.                                                        */
inline wilc_Uint8 *get_address1_ptr(wilc_Uint8* msa)
{
    return (msa + 4);
}
...
/* header and updates the MAC Address in the allocated 'addr' variable.      */
inline void get_address2(wilc_Uint8* msa, wilc_Uint8* addr)
{
    wilc_memcpy(addr, msa + 10, ETH_ALEN);
}

/* This function extracts the MAC Address in 'address3' field of the MAC     */
/* header and updates the MAC Address in the allocated 'addr' variable.      */
inline void get_address3(wilc_Uint8* msa, wilc_Uint8* addr)
{
    wilc_memcpy(addr, msa + 16, ETH_ALEN);
}

/* This function returns the pointer to the MAC Address in 'Address-3" field */
/* of the MAC header.                                                        */
inline wilc_Uint8 *get_address3_ptr(wilc_Uint8* msa)
{
    return (msa + 16);
}
...
inline BASICTYPE_T get_type(wilc_Uint8* header)
{
...
/* Returns the value in the LSB of the returned value.                       */
inline TYPESUBTYPE_T get_sub_type(wilc_Uint8* header)
{
...
/* the input frame.                                                          */
/* Returns the value in the LSB of the returned value.                       */
inline wilc_Uint8 get_pwr_mgt(wilc_Uint8* header)
{
...
/* Returns wilc_TRUE if the input address has the group bit set.                 */
inline wilc_Bool is_group(wilc_Uint8* addr)
{
    if((addr[0] & BIT0) != 0)
        return wilc_TRUE;
...
inline wilc_Bool is_sub_type_null_prot(wilc_Uint16 frm_type)
{
...
/* the input frame.                                                          */
/* Returns the value in the LSB of the returned value.                       */
inline wilc_Uint8 get_protocol_version(wilc_Uint8* header)
{
...
/* returns wilc_TRUE, wilc_FALSE otherwise                                           */
inline wilc_Bool check_from_to_ds(wlan_rx_t *wlan_rx)
{
...
/* Returns wilc_Bool, wilc_TRUE if the two addresses are same and wilc_FALSE otherwise. */
inline wilc_Bool mac_addr_cmp(wilc_Uint8* m1, wilc_Uint8* m2)
{
    if(memcmp(m1, m2, 6) == 0)
        return wilc_TRUE;
...
inline wilc_Bool check_bssid_match(wlan_rx_t *strWlan_rx)
{

	linux_wlan_t *pd = g_linux_wlan;
	wilc_Uint8*  u8macaddr = NULL;  
...
{
    wilc_Uint8 frm_ds = strWlan_rx->u8frm_ds;
    wilc_Uint8 to_ds  = strWlan_rx->u8to_ds;
    if((to_ds == 0) && (frm_ds == 0))
    {
...
inline wilc_Bool is_qos_bit_set(wilc_Uint8* msa)
{
    return (msa[0] & 0x80)?wilc_TRUE:wilc_FALSE;
}
...
inline void set_more_data(wilc_Uint8* header, wilc_Uint8 more_data)
{
    header[1] &= 0xDF;
...
inline void set_qos_prot(wilc_Uint8 *msa)
{
#if 0 //def MAC_WMM
    set_eosp(msa);
#endif /* MAC_WMM */
...
inline void set_dtim_bit(wilc_Uint8 type)
{
    if(type == AID0_BIT)
...
/* to the type given as input.                                               */
inline void reset_dtim_bit(wilc_Uint8 type)
{
    if(type == AID0_BIT)
...
inline wilc_Uint8 get_order_bit(wilc_Uint8 *header)
{
...
inline wilc_Bool is_ht_frame(wilc_Uint8 *header)
{
    if((wilc_TRUE == is_qos_bit_set(header)) && (1 == get_order_bit(header)))
        return wilc_TRUE;
...
inline void get_DA(wilc_Uint8* data, wilc_Uint8* da)
{
    if(get_to_ds(data) == 1)
...
inline wilc_Uint8 *get_DA_ptr(wilc_Uint8* data)
{
    if(get_to_ds(data) == 1)
...
inline wilc_Uint8 get_mac_hdr_len(wilc_Uint8 *msa)
{
    wilc_Uint8 mac_hdr_len = MAC_HDR_LEN;

    /* The MAC Header len is 26 only when in QOD Data frames */
    if((is_qos_bit_set(msa) == wilc_TRUE) && (get_type(msa) == DATA_BASICTYPE))
        mac_hdr_len += QOS_CTRL_FIELD_LEN;


    if(wilc_TRUE == is_ht_frame(msa))
...
/* returns wilc_TRUE if TIM needs to be reset                                 */
inline wilc_Bool update_ps_counts_txcomp(wilc_ieee80211_sta *ae, wilc_Uint8 *msa)
{
    wilc_Bool retval = wilc_FALSE;

    /* TBD: Device a method avoid all this processing in every Tx comp    */
    /* interrupt                                                          */
#if 0//def MAC_WMM
    BOOL_T del_ena = wilc_FALSE;
    if(wilc_TRUE == is_qos_bit_set(msa))
    {
        wilc_Uint8 priority = get_tid_value(msa);

        if(wilc_TRUE == check_ac_is_del_en(ae, priority))
            del_ena = wilc_TRUE;
    }

    if(wilc_TRUE == del_ena)
    {
        ...
        if(ae->USP_in_progress == wilc_TRUE)
        {
            ae->num_USP_pkts_qed--;
            if(ae->num_USP_pkts_qed == 0)
                ae->USP_in_progress = wilc_FALSE;
        }
    }
    else
#endif /* MAC_WMM */
...
inline wilc_Uint16 modify_frame_length( wilc_Uint8 *msa, wilc_Uint16 *rx_len, wilc_Uint16 *data_len)
{ 
    wilc_Uint32 mac_hdr_len = get_mac_hdr_len(msa);
...
inline void flush_ps_queues(wilc_ieee80211_sta *ae)
{
...
inline void handle_ps_sta_change_to_active(wilc_ieee80211_sta *ae)
{
...
/*
    ...
    {
        while((status=requeue_ps_packet(ae, &(ae->ps_q_lgcy), wilc_TRUE, wilc_FALSE))
            == PKT_REQUEUED);
        ...
        if(status == PKT_NOT_REQUEUED)
        {
            handle_requeue_pending_packet(ae, &g_ps_pending_q, wilc_TRUE);
        }
#endif /* PS_DSCR_JIT */
    }
    ...
    if(g_num_sta_ps == 0)
    {
        while(requeue_ps_packet(NULL, &g_mc_q, wilc_TRUE, wilc_FALSE) == PKT_REQUEUED);
    }
}
...
inline void handle_ps_sta_change_to_powersave(wilc_ieee80211_sta *ae)
{
	wilc_Bool ps_del_en_ac = wilc_FALSE;
    ...
    //update_ps_flags_ap(ae, wilc_FALSE, num_elements_moved,ps_del_en_ac);
...
*  @version		1.0
*/
void check_ps_state(wilc_ieee80211_sta* ae, STA_PS_STATE_T ps)
{
...
*  @version		1.0
*/
wilc_Bool filter_wlan_rx_frame(wlan_rx_t *strWlan_rx)
{
    wilc_ieee80211_sta *ae = (wilc_ieee80211_sta *)strWlan_rx->u8sa_entry;
    wilc_Uint8 *msa      = strWlan_rx->u8msa;
...
void wilc_Process_rx_frame(wilc_Uint8 *u8Buff, wilc_Uint32 u32Size)
{
	...
	wilc_Uint32      min_pkt_len = MAC_HDR_LEN ; //null frame
	wilc_Uint8 * mac_addr = g_linux_wlan->nmc_netdev->dev_addr;
	...
	if(check_from_to_ds(&strWlan_rx) == wilc_TRUE) 
	{
	...
	if(check_bssid_match(&strWlan_rx) == wilc_FALSE) 
	{
	   	 if(strWlan_rx.bIs_grp_addr == wilc_FALSE)
	...
	if((strWlan_rx.bIs_grp_addr == wilc_FALSE) &&
	 (mac_addr_cmp(strWlan_rx.u8addr1,mac_addr) == wilc_FALSE) ) 
	 {
		  wilc_Uint8 *addr1 = strWlan_rx.u8addr1;
	...
	if(filter_wlan_rx_frame(&strWlan_rx) == wilc_TRUE) 
	{
		  //g_mac_stats.pewrxft++;
		  return;
	}
...
#ifdef USE_WIRELESS
	    wilc_WFI_monitor_rx(strWlan_rx.u8msa,strWlan_rx.u16rx_len);
#endif
  ...
  /*
	...
	// TODO: update statistics
	    /* Update receive MIB counters */
	   /* if(wlan_rx.is_grp_addr == wilc_TRUE) {
...
void ap_enabled_rx_data(wilc_Uint8* msg)
{
    //wilc_Uint8             priority_rx   = 0;
    STA_PS_STATE_T     ps            = ACTIVE_PS;
    wilc_Uint8             data_trailer  = 0;
    wilc_Uint8*		 buffer = 0 ;
    wilc_Uint8*		host_hdr =0;
    wlan_rx_t          *wlan_rx      = (wlan_rx_t *)msg;
    wilc_Uint8             *msa          = wlan_rx->u8msa;
    wilc_ieee80211_sta       *da_ae        = 0;
    wilc_ieee80211_sta       *sa_ae        = 0;
   ...
   // msdu_desc_t        *frame_desc   = 0;
    ...
    sa_ae = (wilc_ieee80211_sta *)wlan_rx->u8sa_entry;
    ...
    if((frm_type == NULL_FRAME) ||
       (is_sub_type_null_prot(frm_type) == wilc_TRUE))
    {
...
//#endif /* DEBUG_MODE */
   ...
   check_ps_state(sa_ae, ps);
    ...
    /* Create the MSDU descriptors for the received frame */
        ...
        sa_ae  = (wilc_ieee80211_sta* )find_entry(wlan_rx->u8sa);
        da_ae  = (wilc_ieee80211_sta* )find_entry(wlan_rx->u8da);

        if(da_ae != 0)
        {
            //wilc_Uint8 key_type        = 0;
            //wilc_Bool wlan2wlan       = wilc_FALSE;
            wilc_Bool use_same_buffer = wilc_TRUE;
            ...
            if(da_ae->state == ASOC)
            {
				...
				/* header.                                               */
				//if(is_qos_bit_set(msa) == wilc_FALSE)
					//use_same_buffer = wilc_FALSE;
		...
		// send this packet to wlan interface
			...
			tx_data->skb  = NULL;
                wilc_Xmit_data((void*)tx_data, WLAN_TO_WLAN);
        ...
        else
        {
            ...
            if(is_group(wlan_rx->u8da) == wilc_TRUE)
            {
               ...
               // if(ap_check_sec_tx_sta_state(sa_ae) != wilc_TRUE)
                //{
                  //  continue;
                //}
...
//#endif /* DEBUG_MODE */
            ...
            if(wilc_TRUE == is_snap_header_present(wlan_rx))
            {
		...
		wilc_memcpy(host_hdr, wlan_rx->u8da, ETH_ALEN);
        	wilc_memcpy(host_hdr+6, wlan_rx->u8sa, ETH_ALEN);
...
int wilc_Xmit_data(void* ptx_data, Data_Interface InterfaceType)
{
	...
	struct ethhdr 			*eth;
	wilc_ieee80211_sta	*asoc_entry = NULL;
	wilc_Uint8 			ethsaddr[ETH_ALEN];
	wilc_Uint8			ethdaddr[ETH_ALEN];
	wilc_Uint8			mac_hdr_len;
	wilc_Uint8*			da;
	wilc_Uint8	 		service_class, is_qos,is_htc, is_amsdu;
	wilc_Uint8			priority = BEST_EFFORT_PRIORITY; // TODO: this should not be fixed
	wilc_Uint8* 			BSSID = g_linux_wlan->nmc_netdev->dev_addr;
	wilc_Uint8* 			snap_hdr;
	wilc_Uint8* 			mac_hdr;
	wilc_Uint8* 			data_start;
	wilc_Uint16 			eth_type;
	wilc_Uint16 			pkt_len = 0;
	wilc_Uint16 			data_len =0;
	wilc_Uint16			u16size=0;
	...
	if (InterfaceType == HOST_TO_WLAN)
	{
		...
		wilc_memcpy(ethdaddr, &eth->h_dest, ETH_ALEN);

	    	/* For AP, the source address should be sent with the frame. */
		wilc_memcpy(ethsaddr, &eth->h_source, ETH_ALEN);
		...
		/* For unicast packets, check if the destination STA is associated. */
	    	if(is_group(ethdaddr) == wilc_FALSE)
	    	{
		        /* Get association entry for association state */
		        asoc_entry = (wilc_ieee80211_sta*)find_entry(ethdaddr);
	...
	else if(InterfaceType == MONITOR_TO_WLAN)
	{
		...
		/* Get association entry for association state */
		asoc_entry = (wilc_ieee80211_sta*)find_entry(da);

		
		if(buffer_tx_packet(asoc_entry, da, priority , tx_data) == wilc_FALSE)
	    	{
			wilc_wlan_txq_add_mgmt_pkt((void*)tx_data,tx_data->buff,tx_data->size,linux_FH_wlan_tx_complete);
		}
	...
	if (InterfaceType == HOST_TO_WLAN)
	{
	       ...
	       (eth_type == LLTD_TYPE))
	    {
	        ...
	        *snap_hdr++ = 0x00;
	        *snap_hdr++ = 0x00;
		 *snap_hdr++ = (wilc_Uint8)((eth_type>>8) &0x00ff);
		 *snap_hdr++ = (wilc_Uint8)(eth_type &0x00ff);
		...
		wilc_memcpy(data_start, (skb->data) + ETHERNET_HDR_LEN , pkt_len - ETHERNET_HDR_LEN);
	...
	printk("data_len = %d\n",tx_data->size);
	for(i = 0 ; i < tx_data->size ; i++)
		printk("msa2[%d] = %x\n",i,((wilc_Uint8*)(tx_data->buff))[i]);
	}	*/
	
	/*QueueCount = wilc_wlan_txq_add_mgmt_pkt((void*)tx_data,
									tx_data->buff,
									tx_data->size,
									linux_FH_wlan_tx_complete);*/

	//QueueCount = wilc_FH_wlan_txq_add_net_pkt(tx_data,tx_data->buff,tx_data->size,linux_FH_wlan_tx_complete);

	
		  
	if(buffer_tx_packet(asoc_entry, da, priority , tx_data) == wilc_FALSE)
    	{
		QueueCount = wilc_FH_wlan_txq_add_net_pkt((void*)tx_data,
										tx_data->buff,
										tx_data->size,
										linux_FH_wlan_tx_complete);
	}
...
*  @version		1.0
*/
wilc_Uint32 hash(wilc_Uint8* addr)
{
    wilc_Uint32 sum = 0;
    wilc_Uint32  i   = 0;	
...
void* find_entry(wilc_Uint8* key)
{
    ...
    while(tbl_ptr)
    {
        if(mac_addr_cmp(tbl_ptr->key, key) == wilc_TRUE)
...
void add_entry(void* entry, wilc_Uint8* key)
{
    ...
    wilc_memcpy(new_elm->key, key, ETH_ALEN);
...
void handle_ps_tx_comp_ap(struct tx_complete_data *tx_dscr )
{
    wilc_ieee80211_sta *ae;
    wilc_Uint8 *msa  = tx_dscr->buff;
    wilc_Uint8  *da   = NULL;

    /* Get the Destination address */
    da = get_DA_ptr(msa);

    ae = (wilc_ieee80211_sta*)find_entry(da);
	
    if(is_group(da) ==  wilc_TRUE)
    {
    ...
    else
    {
        ...
        {
            if(wilc_TRUE == update_ps_counts_txcomp(ae, msa))
            {
                check_and_reset_tim_bit(ae->u16AID);
            }
        }
...
void check_and_reset_tim_bit(wilc_Uint16 asoc_id)
{
    wilc_Uint8  byte_offset    = 0;
    wilc_Uint8  bit_offset     = 0;
    wilc_Uint8  pvb_offset     = 0;
    wilc_Uint8  length         = 0;
    wilc_Uint8  new_pvb_offset = 0;
    int16_t	 	i              = 0;
    wilc_Uint8  new_length     = MIN_TIM_LEN;
...
{
    wilc_Uint8   q_num        = 0;
    wilc_Uint8   *mac_hdr     = 0;
    wilc_Uint8   addr1[6]     = {0};
    wilc_Uint8   tx_rate      = 0;
    wilc_Uint8   pream        = 0;
    wilc_Uint8   srv_cls      = 0;
    wilc_Uint8   tid          = 0;
    wilc_Uint32  phy_tx_mode  = 0;
    wilc_Bool   is_qos       = wilc_FALSE;
    wilc_Uint32  retry_set[2] = {0};
    ...
    /* If the queue has no packets return wilc_FALSE to indicate that no packets */
	...
	//if((is_qos == wilc_TRUE) && ( !((get_sub_type(mac_hdr) == ACTION ) && (mac_hdr[MAC_HDR_LEN+CATEGORY_OFFSET] == BA_CATEGORY))))
    ...
    /* is returned                                                           */
    if(ps_q_legacy == wilc_FALSE)
    {
        if(is_end_prot(ae) == wilc_TRUE)
        {
            set_qos_prot(mac_hdr);
            end_of_q = wilc_TRUE;
        }
    ...
    if(ps_q_legacy == wilc_TRUE)
    {
	    if(is_group(addr1) == wilc_TRUE)
	    {
	        g_num_mc_bc_pkt--;
	    }
	    else
    ...
    //if(wilc_FALSE == is_serv_cls_buff_pkt((wilc_Uint8 *)ae, q_num, tid, tx_dscr))
    {	

	// TODO: add EAPOL and other monitor types check here
	if(get_sub_type(tx_dscr->buff) == NULL_FRAME)
	{
		wilc_wlan_txq_add_mgmt_pkt((void*)tx_dscr,
									 tx_dscr->buff,
									 tx_dscr->size,
									 linux_FH_wlan_tx_complete);
	}
	...
	{
	    	 wilc_FH_wlan_txq_add_net_pkt((void*)tx_dscr,
										tx_dscr->buff,
										tx_dscr->size,
										linux_FH_wlan_tx_complete);
	}
...
//#ifdef DEBUG_MODE
        //    g_mac_stats.qaexc++;
//#endif /* DEBUG_MODE */
           // free_tx_dscr((wilc_Uint32 *)tx_dscr);
...
*  @version		1.0
*/	
void set_tim_bit(wilc_Uint16 asoc_id)
{
    wilc_Uint8 byte_offset = 0;
    wilc_Uint8 bit_offset  = 0;
    wilc_Uint8 pvb_offset  = 0;
...
*  @version		1.0
*/	
void update_ps_flags_ap(wilc_ieee80211_sta *ae, wilc_Bool bc_mc_pkt,
                        wilc_Uint8 num_buff_added, wilc_Bool ps_add_del_ac)
{
    if(num_buff_added == 0)
        return;

    if(bc_mc_pkt == wilc_TRUE)
    {
...
/*                                                                           */
/*  Returns       : wilc_TRUE, if the packet should be buffered for power save   */
/*                  wilc_FALSE, if the packet should not be buffered             */
/*                                                                           */
/*  Issues        : None                                                     */
/*                                                                           */
/*  Revision History:                                                        */
...
wilc_Bool is_ps_buff_pkt_ap(wilc_ieee80211_sta *ae, wilc_Uint8 *da, struct tx_complete_data* dscr)
{
    wilc_Bool  ps_del_en_ac    = wilc_FALSE;
    wilc_Bool  bc_mc_pkt       = is_group(da);
    wilc_Uint8  priority        = 0;
    list_buff_t *qh         = NULL;
    ...
    if( qh != NULL)
    {
        ...
        update_ps_flags_ap(ae, bc_mc_pkt, 1, wilc_FALSE);
    }
    else
    {
        if(dscr->skb)
...
/*  Outputs       : None                                                     */
/*                                                                           */
/*  Returns       : wilc_TRUE, if the given packet is buffered in any queue      */
/*                  wilc_FALSE, if the packet is not buffered and can be sent    */
/*                                                                           */
/*  Issues        : None                                                     */
/*                                                                           */
/*  Revision History:                                                        */
...
wilc_Bool buffer_tx_packet(wilc_Uint8 *entry, wilc_Uint8 *da, wilc_Uint8 priority,
                         struct tx_complete_data *tx_dscr)
{
    if(wilc_TRUE == is_ps_buff_pkt_ap((wilc_ieee80211_sta*)entry, da, tx_dscr))
    {
#ifdef DEBUG_MODE
        g_mac_stats.psbuff++;
#endif /* DEBUG_MODE */
    ...
    else if(wilc_TRUE == is_serv_cls_buff_pkt(entry, priority, tx_dscr))
    {
#ifdef DEBUG_MODE
    ...
    /* If the packet is buffered return wilc_TRUE */
    return wilc_TRUE;
}
...
void tx_null_frame(wilc_Uint8 *sa, wilc_ieee80211_sta *ae, wilc_Bool is_qos, wilc_Uint8 priority,wilc_Uint8 more_data)
{
    wilc_Uint8  q_num          = 0;
    wilc_Uint8  tx_rate        = 0;
    wilc_Uint8  pream          = 0;
   ...
   tx_data = (struct tx_complete_data*)kmalloc(sizeof(struct tx_complete_data),GFP_ATOMIC);
	...
	len = MAC_HDR_LEN ;//+ FCS_LEN;
       ...
       msa = (wilc_Uint8*)kmalloc(len,GFP_ATOMIC);
	...
	set_frame_control(msa, (wilc_Uint16)NULL_FRAME);
...
wilc_Bool handle_ps_poll(wlan_rx_t    *wlan_rx )
{
    ...
    wilc_ieee80211_sta *ae       = (wilc_ieee80211_sta*)(wlan_rx->u8sa_entry);
    ...
    if(ae->ps_poll_rsp_qed == wilc_TRUE)
    {
    ...
    if(peek_list(&(ae->ps_q_lgcy)) == NULL)
        tx_null_frame(wlan_rx->u8sa, ae, wilc_FALSE, 0, 0);
    else
        requeue_ps_packet(ae, &(ae->ps_q_lgcy), wilc_TRUE, wilc_FALSE) ;

    ae->ps_poll_rsp_qed = wilc_TRUE;
...
wilc_Bool filter_monitor_data_frames(wilc_Uint8 *buf, wilc_Uint16 len)
{
	...
	if(get_type(buf)== DATA_BASICTYPE)
	{
		...
		tx_data->size = len;
		tx_data->skb  = NULL;
                wilc_Xmit_data((void*)tx_data, MONITOR_TO_WLAN);
